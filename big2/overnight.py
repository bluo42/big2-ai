...
* **Wildcard seats** (7% per opponent seat) drawn from the whole model
  collection — scripted regulars, decomposition, CEM/MLP/DMC champions,
  PPO v1, WangBot_v1, the humanlike clone, the exploiter — so the diet
  never goes stale and PPO v1 (the line the humans rate stronger) stays
  a constant presence.
...
def save_latest(net, d_model, heads, layers, path=LATEST, meta=None):
    ...
    payload = {"state_dict": net.state_dict(), "d_model": d_model,
...
def main() -> None:
    ...
    parser.add_argument("--games-target", type=int, default=1_000_000)
    ...
    parser.add_argument("--save-bar", type=float, default=0.5,
                        help="panel score a candidate must beat (and then "
                             "confirm) before it may claim the best file")
